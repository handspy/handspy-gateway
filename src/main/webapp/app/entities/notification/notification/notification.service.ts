...
export class NotificationService {
  ...
  protected convertDateFromClient(notification: INotification): INotification {
    const copy: INotification = Object.assign({}, notification, {
      timestamp: notification.timestamp && notification.timestamp.isValid() ? notification.timestamp.toJSON() : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((notification: INotification) => {
        notification.timestamp = notification.timestamp ? moment(notification.timestamp) : undefined;
      });
    }
    return res;
  }
